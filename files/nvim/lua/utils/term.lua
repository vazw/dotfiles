...
function M.setup(opts)
  ...
  command("ToggleTerminal", function()
    M.toggle()
  end, {})
  ...
  autocmd("TermClose", {
    ...
    nested = true,
    callback = function()
      if s.current.buffer then
        if vim.v.event.status == 0 and vim.api.nvim_buf_is_valid(s.current.buffer) then
...
local function split_fallback(args)
  ...
  vim.cmd(str:format(args.mods.split, args.range[1] or "", args.mods.vertical and "v" or "", "split"))
end
